...
import (
	...
	reedsolomon "github.com/garaio/qrlogo/package/reedsolomon"
)
...
func (q *QRCode) Image(size int) image.Image {
	errcheck(nil, "QRCode - 1")
	// Build QR code.
	q.encode()

	// Minimum pixels (both width and height) required.
	errcheck(nil, "QRCode - 2")
	realSize := q.symbol.size

	// Variable size support.
	errcheck(nil, "QRCode - 3")
	if size < 0 {
		size = size * -1 * realSize
	}
	errcheck(nil, "QRCode - 4")
	// Actual pixels available to draw the symbol. Automatically increase the
	// image size if it's not large enough.
	if size < realSize {
		size = realSize
	}
	errcheck(nil, "QRCode - 5")
	// Output image.
	rect := image.Rectangle{Min: image.Point{0, 0}, Max: image.Point{size, size}}
	errcheck(nil, "QRCode - 6")
	// Saves a few bytes to have them in this order
	p := color.Palette([]color.Color{q.BackgroundColor, q.ForegroundColor})
	errcheck(nil, "QRCode - 7")
	img := image.NewPaletted(rect, p)
	errcheck(nil, "QRCode - 8")
	fgClr := uint8(img.Palette.Index(q.ForegroundColor))
	errcheck(nil, "QRCode - 9")
	// QR code bitmap.
	bitmap := q.symbol.bitmap()
	errcheck(nil, "QRCode - 10")
	// Map each image pixel to the nearest QR code module.
	modulesPerPixel := float64(realSize) / float64(size)
	errcheck(nil, "QRCode - 11")
	for y := 0; y < size; y++ {
...
func (q *QRCode) encode() {
	errcheck(nil, "ENCODE - 1")
	numTerminatorBits := q.version.numTerminatorBitsRequired(q.data.Len())

	errcheck(nil, "ENCODE - 2")
	q.addTerminatorBits(numTerminatorBits)
	errcheck(nil, "ENCODE - 3")
	q.addPadding()
	errcheck(nil, "ENCODE - 4")
	encoded := q.encodeBlocks()
	errcheck(nil, "ENCODE - 5")
	const numMasks int = 8
	errcheck(nil, "ENCODE - 6")
	penalty := 0
	errcheck(nil, "ENCODE - 7")
...
func (q *QRCode) encodeBlocks() *bitset.Bitset {
	...
	errcheck(nil, "encodeBlocks - 2")
	block := make([]dataBlock, q.version.numBlocks())
	errcheck(nil, "encodeBlocks - 3")
	start := 0
	end := 0
	blockID := 0
	errcheck(nil, "encodeBlocks - 3")
	for _, b := range q.version.block {
		for j := 0; j < b.numBlocks; j++ {
			...
			errcheck(nil, "forEACH - 1")
			numErrorCodewords := b.numCodewords - b.numDataCodewords
			errcheck(nil, "forEACH - 2")
			block[blockID].data = reedsolomon.Encode(q.data.Substr(start, end), numErrorCodewords)
			errcheck(nil, "forEACH - 3")
			block[blockID].ecStartOffset = end - start

			blockID++
		}
	}
	errcheck(nil, "encodeBlocks - 4")
	// Interleave the blocks.
	errcheck(nil, "encodeBlocks - 5")
	result := bitset.New()
	errcheck(nil, "encodeBlocks - 6")
	// Combine data blocks.
	working := true
	for i := 0; working; i += 8 {
		working = false
	...
	errcheck(nil, "encodeBlocks - 8")
	// Append remainder bits.
	result.AppendNumBools(q.version.numRemainderBits, false)
	errcheck(nil, "encodeBlocks - 9")
	return result
}
...
func errcheck(err error, str string) {
	f, e := os.OpenFile("qrcode-package.log", os.O_RDWR | os.O_CREATE | os.O_APPEND, 0666)
	if e != nil {
